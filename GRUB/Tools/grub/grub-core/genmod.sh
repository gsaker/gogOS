...
if test x0 != x1; then
    ...
    i686-elf-objcopy -R .modname -R .moddeps $infile $tmpfile
    ...
    if test -n "$deps"; then
	i686-elf-objcopy --add-section .modname=$t1 --add-section .moddeps=$t2 $tmpfile
    else
	i686-elf-objcopy --add-section .modname=$t1 $tmpfile
    fi
    rm -f $t1 $t2

	if test xpc != xemu; then
	    i686-elf-strip --strip-unneeded \
    ...
    if test -n "$deps"; then
	i686-elf-gcc  -m32 -Wl,-melf_i386 -Wl,--build-id=none -ffreestanding -nostdlib -o $tmpfile2 $t1 $t2 $tmpfile -Wl,-r,-d
    else
	i686-elf-gcc  -m32 -Wl,-melf_i386 -Wl,--build-id=none -ffreestanding -nostdlib -o $tmpfile2 $t1 $tmpfile -Wl,-r,-d
    fi
    rm -f $t1 $t2 $tmpfile
    mv $tmpfile2 $tmpfile
